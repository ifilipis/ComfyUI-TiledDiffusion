...
class AbstractDiffusion:
    ...
    def init_grid_bbox(self, tile_w:int, tile_h:int, overlap:int, tile_bs:int):
        ...
        for global_index, bbox in enumerate(bboxes):
            bbox.global_index = global_index
        self.bboxes = bboxes
        self.weights += weights
        self.num_tiles = len(bboxes)
        if self.force_unified_pass and self.num_tiles > 0:
            self.tile_bs = self.num_tiles
            self.num_batches = 1
            self.batched_bboxes = [self.bboxes]
        else:
