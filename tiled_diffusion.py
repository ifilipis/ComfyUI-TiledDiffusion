...
class AbstractDiffusion:
    ...
    def _get_qwen_model(self, model_function):
    ...
    def _patch_qwen_process_img(self, diffusion_model) -> None:
        if hasattr(diffusion_model, "_tilediffusion_process_img_orig"):
            return
        diffusion_model._tilediffusion_process_img_orig = diffusion_model.process_img

        def process_img_wrapped(self, x, index=0, h_offset=0, w_offset=0):
            offsets = getattr(self, "_tilediffusion_offsets", None)
            if offsets is not None:
                index, h_offset, w_offset = offsets
            return self._tilediffusion_process_img_orig(x, index=index, h_offset=h_offset, w_offset=w_offset)

        diffusion_model.process_img = process_img_wrapped

    def _set_qwen_tile_offsets(self, diffusion_model, bbox: BBox, full_h: int, full_w: int) -> None:
        patch_size = getattr(diffusion_model, "patch_size", None)
        if not isinstance(patch_size, int) or patch_size <= 0:
            return
        global_h_len = ((full_h + (patch_size // 2)) // patch_size)
        global_w_len = ((full_w + (patch_size // 2)) // patch_size)
        tile_h_len = ((bbox.h + (patch_size // 2)) // patch_size)
        tile_w_len = ((bbox.w + (patch_size // 2)) // patch_size)
        start_h = ((bbox.y + (patch_size // 2)) // patch_size)
        start_w = ((bbox.x + (patch_size // 2)) // patch_size)
        offset_h = start_h + (tile_h_len // 2) - (global_h_len // 2)
        offset_w = start_w + (tile_w_len // 2) - (global_w_len // 2)
        diffusion_model._tilediffusion_offsets = (0, offset_h * patch_size, offset_w * patch_size)
...
class MultiDiffusion(AbstractDiffusion):
    ...
    def __call__(self, model_function: BaseModel.apply_model, args: dict):
        ...
        qwen_model = self._get_qwen_model(model_function)
        if qwen_model is not None:
            self._patch_qwen_process_img(qwen_model)
        qwen_model = self._get_qwen_model(model_function)
        if qwen_model is not None:
            self._patch_qwen_process_img(qwen_model)
        qwen_model = self._get_qwen_model(model_function)
            ...
            for batch_id, bboxes in enumerate(self.batched_bboxes):
                ...
                if rope_enabled:
                    ...
                    for tile_index, bbox in enumerate(bboxes):
                        ...
                        if qwen_model is not None:
                            self._set_qwen_tile_offsets(qwen_model, bbox, H, W)
                        x_tile_out = model_function(x_tile, t_tile, **c_tile)
                        if qwen_model is not None:
                            qwen_model._tilediffusion_offsets = None
