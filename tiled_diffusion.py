...
class AbstractDiffusion:
    ...
    def _qwen_image_rotary_emb(self, model_function, x_tile: Tensor, context: Tensor, bbox: BBox, full_h: int, full_w: int) -> Tensor | None:
        ...
        if t > 1:
            ...
            img_ids[:, :, :, 0] = img_ids[:, :, :, 0]
        scale_y = (global_h_len - 1) / max(h_len - 1, 1)
        scale_x = (global_w_len - 1) / max(w_len - 1, 1)
        img_ids[:, :, :, 1] = img_ids[:, :, :, 1] + torch.linspace(h_offset * scale_y, (h_len - 1 + h_offset) * scale_y, steps=h_len, device=x_tile.device, dtype=x_tile.dtype).unsqueeze(1).unsqueeze(0) - (global_h_len // 2)
        img_ids[:, :, :, 2] = img_ids[:, :, :, 2] + torch.linspace(w_offset * scale_x, (w_len - 1 + w_offset) * scale_x, steps=w_len, device=x_tile.device, dtype=x_tile.dtype).unsqueeze(0).unsqueeze(0) - (global_w_len // 2)
