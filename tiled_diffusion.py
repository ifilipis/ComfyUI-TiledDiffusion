...
def _patch_qwen_tiled_diffusion():
    ...
    def tiled_forward(
        ...
        **kwargs,
        ...
        tile_h_len = ((x.shape[-2] + (patch_size_int // 2)) // patch_size_int)
        tile_w_len = ((x.shape[-1] + (patch_size_int // 2)) // patch_size_int)
        full_h = global_info.get("latent_height", x.shape[-2])
        full_w = global_info.get("latent_width", x.shape[-1])
        ...
        for i in range(batch):
            ...
            h_shift = float(h_start) - float(full_h_len // 2) + float(tile_h_len // 2)
            w_shift = float(w_start) - float(full_w_len // 2) + float(tile_w_len // 2)
            img_ids[i, :, :, 1] = img_ids[i, :, :, 1] + h_shift
            img_ids[i, :, :, 2] = img_ids[i, :, :, 2] + w_shift
            img_ids[i, :, :, 0] = img_ids[i, :, :, 1] + torch.as_tensor(
                index, dtype=img_ids.dtype, device=img_ids.device
            )
