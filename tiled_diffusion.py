    ...
    def tiled_forward(
        ...
        **kwargs,
        ...
        if offsets_info:
            ...
            tile_h_len = ((orig_shape[-2] + (patch_size_int // 2)) // patch_size_int)
            tile_w_len = ((orig_shape[-1] + (patch_size_int // 2)) // patch_size_int)
            full_h = global_info.get("latent_height", orig_shape[-2])
            full_w = global_info.get("latent_width", orig_shape[-1])
            full_h_len = ((full_h + (patch_size_int // 2)) // patch_size_int)
            full_w_len = ((full_w + (patch_size_int // 2)) // patch_size_int)

            img_ids_view = img_ids[:, :num_embeds, :].view(
                img_ids.shape[0], tile_h_len, tile_w_len, img_ids.shape[-1]
            )
            batch = img_ids_view.shape[0]
            ...
            if len(offsets) == 0:
                offsets = [{"offset_x": 0, "offset_y": 0}] * batch

            for i, off in enumerate(offsets):
                h_offset = off.get("offset_y", 0)
                w_offset = off.get("offset_x", 0)
                h_start = (h_offset + (patch_size_int // 2)) // patch_size_int
                w_start = (w_offset + (patch_size_int // 2)) // patch_size_int
                delta_h = (
                    float(h_start)
                    - (float(full_h_len) / 2.0)
                    + (float(tile_h_len) / 2.0)
                )
                delta_w = (
                    float(w_start)
                    - (float(full_w_len) / 2.0)
                    + (float(tile_w_len) / 2.0)
                )
                img_ids_view[i, :, :, 1] = img_ids_view[i, :, :, 1] + delta_h
                img_ids_view[i, :, :, 2] = img_ids_view[i, :, :, 2] + delta_w

            img_ids = img_ids.clone()
            img_ids[:, :num_embeds, :] = img_ids_view.view(batch, -1, img_ids.shape[-1])
        ...
        txt_start = round(
            max(
                ((x.shape[-1] + (patch_size_int // 2)) // patch_size_int) // 2,
                ((x.shape[-2] + (patch_size_int // 2)) // patch_size_int) // 2,
            )
        )
        txt_ids = (
                ...
                for k, v in c_in.items():
                    if isinstance(v, torch.Tensor):
                        if len(v.shape) == len(x_tile.shape):
                            ...
                            v = torch.cat([
                                v[self._bbox_slicer_for_tensor(bbox_, v)] for bbox_ in bboxes_[batch_id]
                            ])
